...
describe("Componente FoundationCard", () => {
  ...
  it("renderiza nombre, dirección y descripción", () => {
    ...
    expect(
      screen.getByText("Esta es una fundación dedicada a ayudar.")
    ).toBeInTheDocument();
  });
  ...
  it("muestra el badge de verificación si la fundación está verificada", () => {
    render(
      <MemoryRouter>
        <FoundationCard
          foundation={{ ...mockFoundation, verified: true }}
          user={user}
        />
      </MemoryRouter>
    );
  ...
  it("no muestra el botón de favoritos si no hay usuario", () => {
    ...
    expect(
      screen.queryByTestId("mock-favorite-button")
  ...
  it("renderiza los botones con los enlaces correctos", () => {
    ...
    expect(screen.getByText("common.viewDetails")).toHaveAttribute(
      "href",
      "/foundations/1"
    );
    expect(screen.getByText("common.donate")).toHaveAttribute(
      "href",
      "/donate/1"
    );
  });
